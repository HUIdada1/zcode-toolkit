...
加载本插件自带的 `zcode-patcher` 技能（`${CLAUDE_PLUGIN_ROOT}/skills/zcode-patcher/SKILL.md`；若该变量未展开，就按插件安装目录下的同名路径读取），按用户点名的功能执行还原：
...
- 先确认 ZCode 已**完全退出**——重打包级补丁（TPS / 拉取按钮）还原会写 `app.asar`。
