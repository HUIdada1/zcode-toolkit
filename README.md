...
| TPS 状态栏 | 输入框下方居中常驻统计条（右键可切工具栏 / 会话顶部 sticky）：`● 32 tok/s · out 1.7k │ 第 8 轮 │ 输入 45.2k · 命中 38.1k · 平均命中 84% · 累出 12.3k`——左组本轮指标、右组会话累计，空会话空态常驻 | `--tps-footer` |
| 思考强度滑条 | 工具栏「思考 · 档名」入口（迷你电量条），点击弹出吸附拖拽条（模糊渐显、弹性扫入、光点跟随等动效）；档位取自模型实际配置，拖完走原生链路即时生效，原生下拉隐藏、状态双向同步 | `--thought-slider` |
...
skills/zcode-tokenspeed/scripts/
  zcode_patcher.py                            主工具：六个补丁（纯 Python 标准库，解析/重打包 asar 不依赖 Node）
  ...
  zcode-thought-slider.js                     思考强度滑条注入脚本（Codex 风格点击弹出）
...
生效时机分两类：用量图表和弹窗加宽是字节级改写，ZCode 运行中也能写，**下次会话启动即生效**；TPS 状态栏、思考强度滑条和模型拉取按钮要重写 `app.asar`，运行中被文件锁挡住，所以由看护在 **ZCode 退出时自动应用**，下次启动生效。
...
> 思考强度滑条（`--thought-slider`）暂未接入插件开关，需要时直接用命令注入。
