...
| 3 | 在「个人」分段找到 **ZCode Patcher**，点 **安装** |
| 4 | 点开插件 → **高级信息 → 配置**，把要用的功能拨成开 → **保存配置** |
| 5 | **完全退出并重启 ZCode**（托盘右键退出，关窗口不算），功能生效 |
...
4. **拨开关**：点开插件卡片 → 展开底部 **高级信息** → **配置** 区，把想要的功能拨成开 → 点 **保存配置**（详见 [配置步骤](#配置步骤)）。
5. **重启 ZCode**：重打包级功能（状态栏 / 滑条 / 增强提示词 / 拉取按钮）会在 **ZCode 退出时**由插件自动应用，下次启动生效。
...
- 同步日志在插件目录的 `scripts/_sync.log`，出问题时先看它。
