...
| TPS 状态栏 | 输入框下方居中常驻统计条(右键可切工具栏 / 会话顶部):`● 32 tok/s · out 1.7k │ 第 8 轮 │ 输入 45.2k · 命中 38.1k · 平均命中 84% · 累出 12.3k`——左组本轮指标、右组会话累计,空会话空态常驻 | `--tps-footer` |
| 思考强度滑条 | 工具栏「思考 · 档名」入口,点击弹出吸附拖拽条:八帧奔跑小人滑块(拖动加速、松手减速)、换档涟漪、填充弹性扫入;档位取自模型实际配置,拖完即时生效 | `--thought-slider` |
