...
插件在 ZCode 的「设置 → 插件 → 已安装 → 点开插件 → 高级信息 → 配置」里声明了 8 个开关。**不拨也能用**：没保存过配置时按清单默认值（全开）注入。拨动开关并点「保存配置」后，插件会在**下次会话启动时**把客户端同步成你保存的状态，不需要手动跑命令（插件的安装方式见仓库 README 的「安装」章节）：

| 开关 | 控制的功能 | 默认 | 生效时机 |
|---|---|---|---|
| 思考档位配置（3.14+） | `--reasoning-config` | 开 | 下次会话启动（配置侧，无需重启 ZCode） |
| 用量页去截断 | `--usage-chart` | 开 | 下次会话启动（字节级，无需重启 ZCode） |
| 模型弹窗加宽 | `--model-width` | 开 | 下次会话启动（同上） |
...
工具栏常驻「思考 · 档名 ▾」入口（带迷你电量条，造型同原生），点击弹出吸附拖拽条面板（260ms 弹出动效：模糊渐显 + 过冲缩放；填充条弹性扫入，滑块是八帧奔跑小人（拖动加速、松手减速停下，致敬 Codex），换档发射涟漪、档名脉冲，刻度点级联弹入），拖拽/点击/←→键即换档，点外部或 Esc 收起；**原生「思考级别」下拉经 CSS 隐藏（单档位固定徽章除外），走原生切换链路，会话内即时生效、无需重启**：
